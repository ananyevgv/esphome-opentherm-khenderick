...
void OpenThermComponent::setup() {
  ...
  this->request_(OpenThermMessageType::WRITE_DATA, OpenThermMessageID::DEVICE_CONFIGURATION, 0xFFFF);
  ...
  if (this->room_set_temperature_number_) {
    this->room_set_temperature_number_->setup();
    this->room_set_temperature_number_->add_on_state_callback(
        [](float temperature) { ESP_LOGI(TAG, "Request updating Room set setpoint to %f", temperature); });
  }
  if (this->room_temperature_number_) {
    this->room_temperature_number_->setup();
...
void OpenThermComponent::update_spread_() {
  ...
  if (this->otc_ratio_sensor_ && this->should_request_(this->last_millis_otc_ratio_, 23)) {
    this->request_(OpenThermMessageType::READ_DATA, OpenThermMessageID::HCRATIO, 0);
  }    
#endif
